#!/usr/bin/env bash
# SWE coding-agent RL with Qwen3.8-27B + ARCA sandbox, 144 GPUs.
...
# This script uses 128 training GPUs and 16 rollout GPUs; train_async.py does
...
export SLIME_AGENT_ARCA_BASE_URL="${SLIME_AGENT_ARCA_BASE_URL:-http://arca-sandbox.et117-aidc.sh.s-aidc.local:8080}"
...
# ============ Parallelism ============
# Fixed 144-GPU non-colocated layout: 16 nodes x 8 actor GPUs + 16 rollout GPUs.
ACTOR_NUM_NODES=16
...
TRAIN_PROMPT_DATA="${PROMPT_DATA:-/mnt/amedelastic-et117-aidc/common/muchen/datasets/swe_verified_v5.jsonl}"
...
CKPT_ARGS=(
   --hf-checkpoint "${HF_CHECKPOINT:-/mnt/amedelastic-et117-aidc/common/ckpt/AQInfra/Qwen3.8-27B}"
   --ref-load "${REF_MODEL_PATH:-/mnt/amedelastic-et117-aidc/common/ckpt/AQInfra/Qwen3.8-27B_torch_dist}"
   --save "${SAVE_PATH}"
