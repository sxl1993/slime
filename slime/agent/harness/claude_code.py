...
def _claude_run_result(exit_code: int, output_tail: str) -> HarnessRunResult:
    ...
    if error_message and ("output token maximum" in error_message or "CLAUDE_CODE_MAX_OUTPUT_TOKENS" in error_message):
...
class ClaudeCodeHarness(BaseHarness):
    ...
    async def launch_and_wait(
        ...
        for attempt in range(1, self.max_recovery_attempts + 1):
            if result.error_type not in {"max_output_tokens", "context_overflow"}:
                return result
